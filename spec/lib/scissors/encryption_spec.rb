...
describe Scissors::Encryption do
  ...
  before do
    ...
    @early_signature = "\xCBo8\x80\xE1e\xA0\xF4?\xDC\x1E@\xD50\xA9(2\xD5F\xAD\xC7J\x06\xED\xA09\xD0\x8F\xDD$m\xA2"
    @later_signature = "4Ai\x89\x95\xEB~\x84\x16\x7F\x9E\x92\x8Bm\x89M8\xBD8\xFD\xF5\xCB\x05Q~;`C\xFF\xD1\x81#"
  end
  ...
  describe 'signing arbitrary data' do
    ...
    describe 'earlier' do
      ...
      it 'generates a signature' do
        @subject.sign(@my_data).should == @early_signature
      end

      it 'verifies the later signature' do
        @subject.valid?(@my_data, @later_signature).should == true
      end

      it 'verifies the earlier signature' do
        @subject.valid?(@my_data, @early_signature).should == true
      end
    end
    ...
    describe 'later' do
      ...
      it 'generates a different signature' do
        @subject.sign(@my_data).should == @later_signature
      end
